    ...
    execute: function () {
      ...
      _export("WholeDelaunay", WholeDelaunay = (_dec = ccclass('WholeDelaunay'), _dec(_class = class WholeDelaunay {
        ...
        polygonToString(polygon) {
          var output = "this.gr.moveTo(" + this.rnd(polygon[0].x) + "," + this.rnd(polygon[0].y) + ");";
        ...
        findFracture() {
          var points = [];
          var direction = this.invertVector(this.delFracInstance.getNormal());
          var nearestIndex = 0;
          var minDistance = 10000;

          for (var i = 0; i < this.delFracInstance.delaunayPoints.length; i++) {
            var element = this.delFracInstance.delaunayPoints[i];
            var distance = this.lengthVector(this.delFracInstance.contactPoint, new Vec2(element.x, element.y));

            if (distance < minDistance && element.links.length > 0 && element.border) {
              minDistance = distance;
              nearestIndex = i; //  console.log("DISTANCE: " + distance)
          ...
          while (!founded) {
            ...
            var indBestFit = this.findClosestAngle(this.getRelativeAngle(new Vec2(0, 1), direction), actualLinks);
            ...
            var closesdtIndex = this.findClosestIndex(actualLinks[indBestFit], this.delFracInstance.delaunayPoints);
          ...
          var aIndex = this.findClosestIndex(points[0], this.delFracInstance.delaunayPoints);
          var bIndex = this.findClosestIndex(points[points.length - 1], this.delFracInstance.delaunayPoints);
        ...
        getRelativeAngle(vecA, vecB) {
          var rads = this.getRads(vecA); // console.log("BASE ANGLE: "+rads)

          var transformedVectorB = new Vec2(vecB.x * Math.cos(rads) + vecB.y * Math.sin(rads), -1 * vecB.x * Math.sin(rads) + vecB.y * Math.cos(rads)); // console.log("TRANSFORMED VECTOR: "+ transformedVectorB)
        ...
        getRads(vecA) {
          ...
          var baseAngle = Math.abs(Math.atan(vecA.y / vecA.x));
        ...
        */
        ...
        findClosestIndex(pos, pVs) {
          ...
          for (var index = 0; index < pVs.length; index++) {
            var element = pVs[index]; //   console.log("ELEMENT: "+element)
        ...
        unitaryVector(posA, posB) {
          var length = this.lengthVector(posA, posB);
        ...
        getFracturable() {
          ...
          var internals = this.deepCopy(this.delFracInstance.delaunayPoints, false);
          var partA = [];
          var partB = [];
          var externals = this.deepCopy(this.delFracInstance.delaunayPoints, true);
          ...
          while (continuar) {
            if (i == this.delFracInstance.getPointA()) {
              this.delFracInstance.fracturePoints.forEach(element => {
                var k = this.cloneVertexPlain(element);
                k.border = true;
                partA.push(k);
              });

              for (var j = this.delFracInstance.getPointA(); j <= this.delFracInstance.getPointB() - 1; j++) {
                partB.push(this.cloneVertexPlain(externals[j]));
              }

              for (var _j = this.delFracInstance.fracturePoints.length - 1; _j > 0; _j--) {
                var k = this.cloneVertexPlain(this.delFracInstance.fracturePoints[_j]);
                k.border = true;
                partB.push(k);
              }
          ...
          console.log(">>PART A");
          var before = this.deepCopy(this.delFracInstance.delaunayPoints, true);
          console.log("BEFORE ( only externals");
          console.log(before);
          this.removeOutRunners(partA);
          console.log(partA); //  console.log("AFTER")
          // console.log(partA)
        ...
        removeOutRunners(verts) {
          ...
          for (var j = 0; j < cant; ++j) {
            ...
            var v1 = this.unitaryVector(a, b);
            var v2 = this.unitaryVector(a, c);
            var maxAngle = this.getRelativeAngle(v1, v2); //    console.log("POS  x: "+a.x+"  y: "+a.y)
            ...
            if (a.links.length > 0) {
              for (var k = 0; k < a.links.length; k++) {
                if (a.links[k] != null) {
                  ...
                  var angle = this.getRelativeAngle(v1, v2); //    console.log("A LINK item  x: "+a.links[k].x+"   y: "+a.links[k].y)
                  //  console.log(angle)
        ...
        deepCopy(pv, externalBorder) {
          ...
          pv.forEach(pv_el => {
            if (pv_el.border == externalBorder) {
              var newPv = new (_crd && plainVertex === void 0 ? (_reportPossibleCrUseOfplainVertex({
                error: Error()
              }), plainVertex) : plainVertex)(pv_el.x, pv_el.y, pv_el.border);
              var newLinkNodes = [];
              pv_el.links.forEach(ln => {
                if (ln != null) {
                  var newlinkNode = new (_crd && linkNode === void 0 ? (_reportPossibleCrUseOflinkNode({
                    error: Error()
        ...
        cloneVertexPlain(ver) {
          ...
          ver.links.forEach(element => {
            if (element != null) {
              var newLNode = new (_crd && linkNode === void 0 ? (_reportPossibleCrUseOflinkNode({
                error: Error()
        ...
        calculateEnvolArea(indexes, coords, jump) {
          ...
          for (var i = 0; i < indexes.length; i += jump) {
            ...
            var triangleArea = Math.abs(this.cross(x1, y1, x2, y2) / 2);
            area += triangleArea;
          }
...
//# sourceMappingURL=ee86782030da902a687534867a4207b2f6268ba4.js.map