  ...
  return {
    ...
    execute: function () {
      ...
      _export("FractwithAssembler", FractwithAssembler = (_dec = ccclass('FractwithAssembler'), _dec2 = property(Prefab), _dec3 = property(CCInteger), _dec4 = property(CCInteger), _dec(_class = (_class2 = class FractwithAssembler extends Component {
        ...
        createCopy(fracture) {
          console.log("ENTRO");
          var parts = MaskSplitter.lineCutPolygon(fracture);
          ...
          if (parts != null) {
            ...
            console.log("PARTS 1");
            console.log(parts[1]);
            var a1 = MaskSplitter.calcAproxArea(parts[0]);
            var a2 = MaskSplitter.calcAproxArea(parts[1]);

            if (a1 > this.minArea && a2 > this.minArea) {
              this.recursiveness--;
              var fi = this.node.getComponent(_crd && FracturaImplementer === void 0 ? (_reportPossibleCrUseOfFracturaImplementer({
                error: Error()
              }), FracturaImplementer) : FracturaImplementer);
              var pc = this.node.getComponent(PolygonCollider2D);
              pc.points = parts[0];
              fi.polygon = parts[0];
              fi.modifyPoints();
              pc.apply();
              var copyOf = instantiate(this.prefab);
              var cfwa = copyOf.getComponent(FractwithAssembler);
        ...
        onBeginContact(selfCollider, otherCollider, contact) {
          var otherFracturable = otherCollider.getComponent(FractwithAssembler); // if (otherCollider.node.name == "Stomper") {
          ...
          if ((otherCollider.node.name == "Stomper" || this.recursiveness > 0 && otherCollider.node.getComponent(FractwithAssembler) != null) && this.area > this.minArea) {
            ...
            var fractureObj = new MaskSplitter.Fracture(localContactPoint, normalContactPoint, this.node.getComponent(PolygonCollider2D).points);
            var fracturePoints = MaskSplitter.getFracturable(fractureObj);
...
//# sourceMappingURL=eac38055539405d98db25eb668a67861d07273c9.js.map