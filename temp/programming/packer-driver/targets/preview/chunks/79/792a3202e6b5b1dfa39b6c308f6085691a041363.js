System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  function findFracture(fracture) {
    ...
    while (!founded) {
      ...
      var relAngle = (_crd && getRelativeAngle === void 0 ? (_reportPossibleCrUseOfgetRelativeAngle({
        error: Error()
      }), getRelativeAngle) : getRelativeAngle)(new Vec2(1, 0), direction);
      console.log("REL ANGLE: " + relAngle);
      console.log("ACTUAL LINKS");
      console.log(actualLinks);
      var indBestFit = findClosestAngle(relAngle, actualLinks);
  ...
  function redireccion(vecA, vecB) {
    var nuevaDir = new Vec2(vecB.x - vecA.x, vecB.y - vecA.y);
  ...
  function removeVertexs(targets, container) {
    for (var i = 0; i < targets.length; i++) {
      var index = indexOfVertex(targets[i], container);
  ...
  function calculateEnvolArea2D(indexes, coords) {
    var area = 0;

    for (var i = 0; i < indexes.length; i += 3) {
      var x1 = coords[indexes[i + 1] * 2] - coords[indexes[i] * 2];
      var y1 = coords[indexes[i + 1] * 2 + 1] - coords[indexes[i] * 2 + 1];
      var x2 = coords[indexes[i + 2] * 2] - coords[indexes[i] * 2];
      var y2 = coords[indexes[i + 2] * 2 + 1] - coords[indexes[i] * 2 + 1];
      var triangleArea = Math.abs(cross(x1, y1, x2, y2) / 2);
      area += triangleArea;
    }
  ...
  _export({
    ...
    calculateEnvolArea2D: calculateEnvolArea2D,
...
//# sourceMappingURL=792a3202e6b5b1dfa39b6c308f6085691a041363.js.map